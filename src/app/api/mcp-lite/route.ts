...
// ============================================
// Lite Mode - App Store Submission (4 tools only)
// ============================================
const LITE_TOOLS = [
...
export async function GET(request: NextRequest) {
  ...
  console.log('[MCP-Lite] GET request received');
  
  // ✅ Lite mode: Fixed 4 tools only (App Store submission)
  return json200({
    tools: [
      {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.method === "tools/call") {
      ...
      // ✅ Lite mode: Allowlist check (must be first check)
      if (name && !LITE_TOOLS.includes(name as any)) {
        return json200({
          jsonrpc: "2.0",
          id: body.id ?? null,
          error: {
...
// ============================================
    ...
    body: JSON.stringify({
        ...
        // ============================================
          ...
          // Fix: For localhost proxy, use 127.0.0.1 instead of localhost to avoid IPv6/IPv4 issues
          let targetUrl: string;
          if (isVercelProxy) {
            // Replace localhost with 127.0.0.1 for server-side fetch
            targetUrl = mcpCareerUrl.replace('localhost', '127.0.0.1');
          } else {
            targetUrl = `${mcpCareerUrl}/api/career/advice`;
          }
