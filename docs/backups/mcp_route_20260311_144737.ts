...
// ============================================
          ...
          const { 
            ...
            source: argsSource   // 'manus' = Manus Customer API: fixed 50 jobs when profile complete
          } = args;
          // Manus caller: header takes precedence, then args.source
          const callerManus = request.headers.get('x-caller')?.toLowerCase() === 'manus'
            || request.headers.get('x-client')?.toLowerCase() === 'manus'
            || argsSource === 'manus';
          // "Complete profile" for Manus: has target position (jobTitles or expectedPosition) and some skills
          const hasTargetPosition = !!(user_profile?.jobTitles?.length || user_profile?.expectedPosition);
          const hasSkills = !!(user_profile?.skills?.length);
          const profileCompleteForManus = hasTargetPosition && hasSkills;
          // When Manus + complete profile: return exactly 50 jobs for batch apply; otherwise use limit (GPT path)
          const effectiveLimit = (callerManus && profileCompleteForManus) ? 50 : limit;
          
          console.log('[MCP] recommend_jobs - exclude_ids:', exclude_ids.length);
          console.log('[MCP] recommend_jobs - caller_manus:', callerManus, 'profile_complete_for_manus:', profileCompleteForManus, 'effective_limit:', effectiveLimit);
          ...
          try {
            ...
            // Fetch enough candidates: for Manus fixed-50 mode use larger pool; otherwise limit*3 / 40
            const searchLimit = effectiveLimit >= 50 ? Math.max(effectiveLimit * 3, 180) : Math.max(limit * 3, 40);
