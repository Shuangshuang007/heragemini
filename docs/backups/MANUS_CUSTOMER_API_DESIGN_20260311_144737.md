...
| **行为差异** | 按对话一次推少量（如 5～20 条） | 在「有完整简历」等条件下**固定返回 50 条**，供 Manus 并发投递 |
...
- **额外工作**：识别 Manus → 在 `recommend_jobs` 里在“有完整简历”时固定返回 50 条；其余逻辑复用即可。

### Manus 内搜索 → 固定 50 条推荐（已实现）

- **场景**：用户在 Manus 里搜索职位，Manus 调 Hera 的 `recommend_jobs` 拿推荐。
- **约定**：当**调用方是 Manus** 且**用户档案完整**时，固定返回 **50 个职位**，方便 Manus 做并发投递；其他情况仍按原有 limit（如 GPT 用 5～20）。
- **识别 Manus**：任选其一即可  
  - 请求头：`X-Caller: manus` 或 `X-Client: manus`  
  - 或参数：`source: 'manus'`
- **“完整简历/档案”**：当前定义为同时具备  
  - 目标职位：`user_profile.jobTitles` 或 `user_profile.expectedPosition`  
  - 技能：`user_profile.skills` 非空  
- **实现位置**：`src/app/api/mcp/route.ts` 中 `recommend_jobs`：`effectiveLimit = (callerManus && profileCompleteForManus) ? 50 : limit`，并相应加大 `searchLimit` 以拉取足够候选。
