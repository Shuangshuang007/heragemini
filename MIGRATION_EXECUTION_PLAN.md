...
12. Step 11 – 统一搜索入口（Job Title 支持公司名称搜索）
13. 附录：验证清单模板
...
**目标**：在 job title 搜索框中同时支持搜索职位名称和公司名称，实现类似 Seek 的统一搜索体验。

### 11.1 需求说明

**当前状态**：
- `search_jobs` 工具只支持 `job_title` 和 `city` 参数
- `search_jobs_by_company` 工具专门用于公司搜索
- 用户需要区分使用哪个工具，体验不够统一

**目标状态**：
- 在 job title 搜索框中可以输入职位名称或公司名称
- 支持混合输入（如 "software engineer at Google"）
- 后端自动识别输入类型并调用相应的查询逻辑
- 前端统一搜索体验，无需用户区分
...
**1. 扩展 `search_jobs` 工具**：
- 在 `inputSchema` 中添加可选的 `company` 参数
- 修改 `handleSearchJobs` 函数，同时支持 `job_title` 和 `company` 参数
- 查询逻辑：
  - 如果同时提供 `job_title` 和 `company`：使用 AND 条件（职位 + 公司）
  - 如果只提供 `job_title`：按职位搜索（现有逻辑）
  - 如果只提供 `company`：按公司搜索（复用 `search_jobs_by_company` 逻辑）

**2. 智能参数解析**：
- 复用现有的 `fixArgs` 函数，自动识别输入是职位名还是公司名
- 支持混合输入解析：
  - "software engineer at Google" → `job_title: "software engineer"`, `company: "Google"`
  - "Google" → `company: "Google"`
  - "marketing manager" → `job_title: "marketing manager"`

**3. 数据库查询优化**：
- `queryJobsWithFilters` 已支持同时传入 `jobTitle` 和 `company`
- 确保查询逻辑正确处理组合条件
- 验证索引性能（`title`、`company`、`locations` 字段）

**涉及文件**：
- `src/app/api/mcp/route.ts`：扩展 `search_jobs` 工具
- `src/services/jobDatabaseService.ts`：验证 `queryJobsWithFilters` 支持组合查询

#### 11.2.2 前端修改

**1. 统一搜索框**：
- 修改搜索输入框的 placeholder：`"Job title, company, or keywords"`
- 支持用户输入职位名、公司名或混合输入

**2. 输入解析逻辑**：
- 创建输入解析函数 `parseSearchInput(input: string)`
- 检测模式：
  - "at/in <Company>" → 识别为公司
  - "<JobTitle> at <Company>" → 识别为混合输入
  - "<Company> in <City>" → 识别为公司+地点
  - 纯文本 → 优先职位，如果匹配公司列表则识别为公司

**3. 搜索逻辑**：
- 解析用户输入，提取 `job_title` 和 `company`
- 统一调用 `search_jobs` API，传入解析后的参数
- 无需区分调用 `search_jobs` 还是 `search_jobs_by_company`

**涉及文件**：
- `src/components/SearchBar.tsx`：更新搜索框和解析逻辑
- `src/app/jobs/page.tsx`：更新搜索处理逻辑

### 11.3 实施步骤

1. **后端扩展**（优先级：高）
   - [ ] 扩展 `search_jobs` 工具的 `inputSchema`，添加 `company` 参数
   - [ ] 修改 `handleSearchJobs` 函数，支持 `company` 参数
   - [ ] 验证 `queryJobsWithFilters` 正确处理组合查询
   - [ ] 测试：只提供 `job_title`、只提供 `company`、同时提供两者

2. **智能解析**（优先级：高）
   - [ ] 优化 `fixArgs` 函数，提升公司名称识别准确率
   - [ ] 支持混合输入解析（"job at company" 格式）
   - [ ] 测试各种输入格式的解析准确性

3. **前端统一**（优先级：中）
   - [ ] 更新搜索框 placeholder 和提示文本
   - [ ] 实现输入解析函数
   - [ ] 更新搜索逻辑，统一调用 `search_jobs` API
   - [ ] 测试前端搜索体验

4. **UI 优化**（优先级：低）
   - [ ] 考虑添加自动补全功能（职位和公司建议）
   - [ ] 参考 Seek 的搜索体验，优化交互细节
...
- [ ] **只提供 job_title**：
  - [ ] `search_jobs({ job_title: "marketing manager", city: "New York" })` → 返回职位搜索结果
  - [ ] 验证查询逻辑与现有行为一致

- [ ] **只提供 company**：
  - [ ] `search_jobs({ company: "Google", city: "New York" })` → 返回公司搜索结果
  - [ ] 验证结果与 `search_jobs_by_company` 一致

- [ ] **同时提供两者**：
  - [ ] `search_jobs({ job_title: "software engineer", company: "Google", city: "New York" })` → 返回 Google 的软件工程师职位
  - [ ] 验证查询使用 AND 条件

- [ ] **智能解析测试**：
  - [ ] `fixArgs({ job_title: "Google Software Engineer" })` → 识别为 `company: "Google"`, `job_title: "Software Engineer"`
  - [ ] `fixArgs({ job_title: "software engineer at Microsoft" })` → 解析为混合输入

#### 11.4.2 前端验证

- [ ] **职位搜索**：
  - [ ] 输入 "marketing manager"，选择 "New York"
  - [ ] 验证返回职位搜索结果

- [ ] **公司搜索**：
  - [ ] 输入 "Google"，选择 "New York"
  - [ ] 验证返回 Google 的职位列表

- [ ] **混合搜索**：
  - [ ] 输入 "software engineer at Google"，选择 "New York"
  - [ ] 验证返回 Google 的软件工程师职位
...
- [ ] **查询性能**：
  - [ ] 组合查询（job_title + company）响应时间 < 2 秒
  - [ ] 数据库索引使用正确（explain 查询）
...
- [ ] **功能验收**：
  - [ ] 在 job title 搜索框中可以输入职位名称
  - [ ] 在 job title 搜索框中可以输入公司名称
  - [ ] 支持混合输入（"job at company" 格式）
  - [ ] 搜索结果正确（职位搜索、公司搜索、混合搜索）

- [ ] **体验验收**：
  - [ ] 用户无需区分使用哪个工具
  - [ ] 搜索框提示清晰（"Job title, company, or keywords"）
  - [ ] 搜索结果与预期一致

- [ ] **技术验收**：
  - [ ] 后端 API 支持 `company` 参数
  - [ ] 智能解析准确率 > 90%
  - [ ] 查询性能满足要求
  - [ ] 代码质量通过 lint 和类型检查
...
**改进前**：
- 用户需要知道使用 `search_jobs` 还是 `search_jobs_by_company`
- 搜索框只能输入职位名称
- 搜索公司需要切换到专门的工具
...
**优势**：
1. **用户体验**：一个搜索框，支持多种输入模式
2. **灵活性**：支持职位、公司、混合搜索
3. **向后兼容**：保留现有工具，不影响 MCP 调用
4. **技术复用**：后端已支持组合查询，主要在前端解析
